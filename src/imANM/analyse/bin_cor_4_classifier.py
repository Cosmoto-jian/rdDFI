...
OUT_DIR    = "results/bin_cor_4_classifier"
S_DEFAULT  = 16
K_DEFAULT  = 1000
N_RES_MIN  = 375
N_RES_MAX  = 475
...
CAT_COLORS  = {"GPCR": "#E63946", "large": "#264653",
               "medium": "#F4A261", "single_pass": "#2A9D8F"}
CAT_MARKERS = {"GPCR": "o", "large": "s", "medium": "^", "single_pass": "D"}
C_NULL = "#999999"
MM = 1.0 / 25.4
...
def load_corr_points_normalized(h5_path, s, K, direction, norm_attr="L_xy"):
    ...
    """
    curve  = {cat: defaultdict(list) for cat in CATEGORIES}
...
def plot_direction(curve, counts, direction, s, K, out_dir, xmax=None):
    """一个方向的 ρ(r) 分类均值曲线 + ±1 SD 色带（归一化距离版）。"""
    ...
    for cat in CATEGORIES:
        if counts[cat] == 0:
            continue
        bins = sorted(curve[cat].keys())
        r_mean, rho_mean, rho_sd = [], [], []
        for r_key in bins:
            vals = np.array(curve[cat][r_key])
            n = len(vals)
            if n < 3:
                continue
            r_mean.append(r_key)
        ...
        ax.fill_between(r_mean, lo_sd, hi_sd,
                        color=CAT_COLORS[cat], alpha=0.13, lw=0, zorder=2)
        # 均值线 + marker
        ax.plot(r_mean, rho_mean, "-", marker=CAT_MARKERS[cat],
                lw=1.4, color=CAT_COLORS[cat],
                markersize=4.0,
                markeredgewidth=0.3, markeredgecolor="black",
                zorder=4, label=CAT_LABELS.get(cat, cat))
    ...
    for cat in CATEGORIES:
        if counts[cat] == 0:
            continue
        for r_key in sorted(curve[cat].keys()):
            if r_key > rmax:
                break
            vals = np.array(curve[cat][r_key])
            if len(vals) < 3:
...
def main():
    ...
    for direction, norm_key, norm_attr, xmax in [
        ...
        for cat in CATEGORIES:
            print(f"   {CAT_LABELS.get(cat, cat):12s}: {counts_norm[cat]:4d} proteins, "
                  f"{sum(len(v) for v in curve_norm[cat].values()):7d} points")
