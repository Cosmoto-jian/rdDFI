...
def plot_figure5(protein_key="acm1", summary_csv="Prediction_models_hit_summary_2.csv"):
    ...
    circle_dfi = patches.Circle((0.40, 0.5), 0.33, color=c_left, alpha=0.5,
                                transform=ax_venn.transAxes, edgecolor="grey", linewidth=1.5, clip_on=False)
    circle_h   = patches.Circle((0.60, 0.5), 0.33, color=c_right, alpha=0.5,
                                transform=ax_venn.transAxes, edgecolor="grey", linewidth=1.5, clip_on=False)
    ax_venn.add_patch(circle_dfi)
    ax_venn.add_patch(circle_h)

    def get_random_points(region, n_points):
        pts = []
        while len(pts) < n_points:
            x = random.uniform(0.0, 1.0)
            y = random.uniform(0.15, 0.85)
            d_left  = math.hypot(x - 0.40, y - 0.5)
            d_right = math.hypot(x - 0.60, y - 0.5)
            in_left  = d_left < 0.28
            in_right = d_right < 0.28
            if region == 'left' and in_left and not in_right:
                pts.append((x, y))
            elif region == 'right' and in_right and not in_left:
                pts.append((x, y))
            elif region == 'both' and in_left and in_right:
                pts.append((x, y))
        return pts

    random.seed(42)
    pts_left  = get_random_points('left', 30)
    pts_right = get_random_points('right', 30)
    pts_both  = get_random_points('both', 15)

    for x, y in pts_left + pts_right + pts_both:
        ax_venn.plot(x, y, marker='o', markersize=1.5,
    ...
    ax_venn.text(0.18, 0.50, f"{dfi_only_count}", ha='center', va='center',
                 fontsize=13, fontweight='bold', color='black', transform=ax_venn.transAxes, zorder=5)
    ax_venn.text(0.82, 0.50, f"{h_only_count}", ha='center', va='center',
                 fontsize=13, fontweight='bold', color='black', transform=ax_venn.transAxes, zorder=5)
    ax_venn.text(0.50, 0.50, f"{both_count}", ha='center', va='center',
                 fontsize=13, fontweight='bold', color='black', transform=ax_venn.transAxes, zorder=5)
